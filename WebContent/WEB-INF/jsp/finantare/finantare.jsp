...
<c:forEach items="${model.finantare}" var="f">
<tr>
 <td><c:out value="${f.id}"/></td>
 <td><c:out value="${f.nume}"/></td>
  <td><c:out value= "${f.bancaEmitenta}"/></td>
  <td><c:out value= "${f.nrRate}"/></td>
   <td><c:out value= "${f.categorieProduse}"/></td>
 <td><c:out value= "${f.campanie}"/></td>
 <td><a href="./finantareEdit/${f.id}" class="far fa-edit"></a>
      |     <a href="deleteFinantare/${f.id}" onclick="return confirm('Sunteti sigur ca doriti sa stergeti aceasta intrare?');"  class="fa fa-trash-o"></a></td>
 </tr>
 <br/>
</c:forEach>
</table>
