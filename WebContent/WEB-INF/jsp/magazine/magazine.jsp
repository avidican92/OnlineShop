...
<c:forEach items="${model.magazine}" var="m">
<tr>
 <td><c:out value="${m.id}"/></td>
 <td><c:out value="${m.nume}"/></td>
 <td><c:out value= "${m.easybox}"/></td>
 <td><c:out value= "${m.adresa}"/></td>
 <td><c:out value= "${m.program}"/></td>
 <td><c:out value= "${m.telefon}"/></td>
 <td><a href="./magazineEdit/${m.id}" class="far fa-edit"></a>
      |     <a href="deleteMagazine/${m.id}" onclick="return confirm('Sunteti sigur ca doriti sa stergeti aceasta intrare?');"  class="fa fa-trash-o"></a></td>
 </tr>
 <br/>
</c:forEach>
</table>
